...
def safe_rerun():
    """Safely rerun the app without causing infinite loops."""
...
def main():
    try:
        ...
        if st.session_state.teaching_state == 'initialize':
            if topic and not st.session_state.curriculum_generated:
            ...
            if not st.session_state.get('lesson_generated', False):
